function [X, iter, out] = gl_gurobi(x0, A, b, mu, opts)
    ...
    model = struct();
    % variables: x11, x21, ..., xn1, x12, x22, ..., xn2, ..., x1l, x2l, ..., xnl, y11, y21, ..., ym1, y12, y22, ..., ym2, ..., yml, t, z1, z2, ..., zn
    model.varnames = cell(1, n*l + m*l + 1 + n );
    for j=1:l
        for i=1:n
    ...
    model.varnames{n*l+m*l+1} = 't';
    for i=1:n
        model.varnames{n*l+m*l+1+i} = ['z' num2str(i)];
    end

    % Objective: 0.5 * sum(y.^2) + mu * sum(z)
    % Define the quad part
    quadrature = zeros(n*l+m*l+1+n, n*l+m*l+1+n);
    quadrature(n*l+1:n*l+m*l, n*l+1:n*l+m*l) = 0.5*eye(m*l);
    model.Q = sparse(quadrature);
    % Define the linear part
    model.obj = zeros(1, n*l+m*l+1+n);
    model.obj(n*l+m*l+2:end) = mu;

    % Linear constraints: (I_l⊗A)*vec(x)−vec(y)=vec(b)
    Aeq = kron(eye(l), A);
    beq = b(:);
    model.A = sparse([Aeq, -eye(m*l), zeros(m*l, 1),zeros(m*l, n)]);
    model.rhs = beq;
    model.sense = '=';
    ...
    for i=1:n
        ...
        constraint_matrix = sparse([elementary_i_diag_cat, zeros(n*l,m*l+1+n); zeros(m*l+1,n*l+m*l+1+n); zeros(n,n*l+m*l+1), -elementary_i]);
        model.quadcon(i).Qc = constraint_matrix;
        model.quadcon(i).q = zeros(n*l+m*l+1+n, 1);
