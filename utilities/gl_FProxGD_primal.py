...
def gl_FProxGD_primal_inner(initial_x: np.ndarray, A: np.ndarray, b: np.ndarray, mu: float, opts: dict = {}):
    ...
    for iteration in np.arange(opts['maxit_inn']):
        ...
        if iteration > 2 and abs(output['f_hist_inner'][iteration] - output['f_hist_inner'][iteration - 1]) < opts['ftol'] and output['g_hist'][iteration] < opts['gtol']:
            output['flag'] = 1
            break
...
def gl_FProxGD_primal(initial_x: np.ndarray, A: np.ndarray, b: np.ndarray, mu: float, opts: dict = {}):
