function [x, num_iters, out] = gl_cvx_mosek(initial_x, A, b, mu, opts)
    % Solve the group LASSO problem using CVX with the MOSEK solver.
    %
    % Args:
    ...
    if isempty(iterations)
        fprintf('ERROR: Solver cvx(MOSEK) recorded zero iterations. Please check stdout redirection!\n');
        num_iters = -1;
    else
